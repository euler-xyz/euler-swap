...
description: "EulerSwap topics relevant for developers and builders"
sidebar_position: 9
---
...
* `borrowVault0` and `borrowVault1`: Addresses of vaults that should be borrowed from once the corresponding supply vault is exhausted. These can be the same addresses as the supply vaults. If `address(0)` is provided, then any operation that attempts a borrow will fail.
...
* `priceX` and `priceY`: These form the numerator and denominator of a fraction that represents the price of the assets at the equilibrium point. This fraction must also reflect decimal differences between the two assets.
...
* `fee0` and `fee1`: The fee to be applied to the input of asset0 and asset1 respectively. These are 18-scale decimal numbers. The special value of `1e18` means that swaps in this direction are rejected. These can be overridden by the [before swap hook](#before-swap-hook).
...
Note that in the initial configuration, these values are verified to represent a point exactly on the curve.
...
The `EulerSwapRegistry` contract is an optional directory that pools can be added to. Only the creator of a pool can add it. By adding a pool to the registry, you are advertising it to solvers and aggregators. Pools in the registry are discoverable by trading pair. Although some solvers may be able to find and use pools that have not been added to the registry, others rely on a more organised and searchable directory of pools.

When adding a pool to the registry, you may be required to post a *validity bond*. This is a bond denominated in native token, the minimum value of which is set by a special registry curator. If you remove your pool from the registry, the bond will be returned to you. However, if at any time your pool is quoting swaps that cannot actually be filled, you may forfeit the bond.
...
* Any user may *challenge* the pool, by providing a quote that cannot be filled. If the challenge is successful, the pool is unregistered and the challenger receives the bond.
...
* The swap is actually performed, by taking the input tokens from the challenger. The challenger must've given appropriate token approval to the registry.
  * If this swap succeeds, the entire transaction is reverted (including the swap) and the challenge is rejected.
  * If the swap failed for any reason other than `E_AccountLiquidity()` then the challenge is rejected. This check is necessary because some vaults can fail for other expected reasons, such as unpopulated pull oracles.
...
The `swapHookedOperations` is a bitmask that controls which of the two hooks should be invoked. The following values are supported:

* `1`: The before swap hook is invoked.
* `2`: The after swap hook is invoked.
* `3`: Both the before and after swap hooks are invoked.

### Before Swap Hook

This hook is invoked at two cases:

* When a quote is being calculated. Since quotes are provided by view methods, the before swap hook must not modify any storage in this case. The hook receives a `readOnly` boolean parameter which indicates this is the case.
* When a swap is about to be performed. `readOnly` will be false in this case, so storage can be modified.

In either case, the before swap hook should return the fee that will be required for the swap. This is a fraction scaled by 18 decimals. For example, a fee of 10% would be `0.1e18`. In addition, there are two special additional values supported:
...
The same warning about verifying `msg.sender` in the the before swap hook also applies to the after swap hook.
